...
namespace com.okcoin.rest
{
    ...
    class Client
    {
        static List<order> parseOrders(string orders)
        {
            orders = orders.Replace("[", "");
            orders = orders.Replace("]", "");
            orders = orders.Replace("}", "");
            string[] pricesAndVolumes = orders.Split(',');
            int orderCount = pricesAndVolumes.GetLength(0) / 2;
            List<order> ret = new List<order>();
            for (int i = 0; i < orderCount; i++)
            {
                order o = new order();
                o.price = float.Parse(pricesAndVolumes[i * 2 + 0]);
                o.volume = float.Parse(pricesAndVolumes[i * 2 + 1]);
                ret.Add(o);
            }
            return ret;
        }
