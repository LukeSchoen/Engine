...
void GenerateDynamicTreeModel(vec3 campos, float camRes, Octree *tree, Node node, vec3 position, int maximumDepth, ColouredRenderObjectMaker *maker, int currentLayer = 0)
{
  ...
  if (layerSize / (position - (vec3() - campos)).Length() < 1.0 / camRes)
  {
...
{
  vec3 lastCamPos = Camera::Position();
  while (true)
  {
    Sleep(10);
    if ((Camera::Position() - lastCamPos).Length() > 0.1)
    {
...
void OpenGL()
{
...
#else
  Window window("Game", true, 1600, 800, false); // Create Game Window
#endif
  ...
  std::thread thread(&CloudStreamThread, &tree, (window.width * 1.5) / (PointSize), (int)11);
