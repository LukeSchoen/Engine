...
const static Material dirt({ 150 / 255.6f, 80 / 255.6f, 20 / 255.6f }, true, true, false, 0.5f, 1.0f);
...
void AmbientOcclusion()
{
  for (int64_t p = 0; p < posData.size(); p++)
  {
    ...
    float m = std::min(8.0 / occlusion, 1.0);
...
void Start()
{
  ...
  if (false)
  {
    for (int y = 0; y < 16; y++)
      for (int x = 0; x < 16; x++)
      {
        ...
        for (int z = 0; z < h; z++)
        {
          //auto col = vec3{ (150 + (rand() % 15)) / 255.6f, (80 + (rand() % 8)) / 255.6f, (20 + (rand() % 2)) / 255.6f };
          auto col = vec3{ 150 / 255.6f, 80 / 255.6f, 20 / 255.6f };
          for (int iy = y * 2; iy < y * 2 + 2; iy++)
            for (int ix = x * 2; ix < x * 2 + 2; ix++)
            AddVoxel({ (float)ix, (float)z, (float)iy }, col, {}, 0.5f, false);
        }
  ...
  if (true)
  {
    for (int i = 0; i < 3; i++)
    {
      ...
      AddVoxel(off + vec3(32 + 5 + 6, 42 + 30, 32 + 5), vec3(1.0, 1.0, 1.0), vec3(), 0);
      AddBox(off + vec3(32 + 5, 42 + 5, 32 + 5), vec3(1, 25, 1), wood);
      AddBox(off + vec3(32 + 5 + 6, 42 + 5, 32 + 5), vec3(1, 25, 1), wood);
      AddBox(off + vec3(32 + 5, 42 -2, 32 + 5), vec3(7, 7, 7), wood);
    }
  }
  ...
  for (int z = 1000; z < 1010; z++)
    for (int y = 0; y < 10; y++)
      for (int x = 1000; x < 1010; x++)
        points.emplace_back(vec3(x, y, z), 255 * 256);
